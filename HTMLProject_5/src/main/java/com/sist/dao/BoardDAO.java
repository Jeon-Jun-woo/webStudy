...
public class BoardDAO {
	 ...
	 *  3. 미리 연결했기 때문에 연결에 소모되는 시간을 줄임
	 *  4. 웹 프로그램에 접근하는 인원이  많아서 쉽게 다운되지않는다
	...
	public void getConnection()
	{
		try
		{
			...
			//2. 저장된 위치 확인
			//3. 실제 Connection주소 얻어오기
			DataSource ds=(DataSource)c.lookup("jdbc/oracle");
			// 4. Conenction의 정보를 받아온다
	...
	public List<BoardVO> boardListData(int page)
	{
		...
		try
		{
			...
			String sql="SELECT no,subject,name,regdate,hit, num "
					...
					+"FROM (SELECT /*+ INDEX_DESC(board board1_no_pk)*/no,subject,name,regdate,hit "
					+"FROM board)) "
