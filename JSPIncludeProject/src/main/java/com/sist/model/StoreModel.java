 ...
 * 		Controller => Model에서 모아둔 request 를 JSP로 전송
 * 		
 * 
 */
...
public class StoreModel {
			...
			HttpServletResponse response)
	{
		...
		if(page==null) //처음은 페이지값을 안보내주니까 초기값 설정
			page="1";
	...
	@RequestMapping("store/detail_before.do") //쿠키 전송용
			...
			HttpServletResponse response)
	{
		...
		return "redirect:../store/detail.do?no="+no; // detail.do를 다시시작
	}
	
		
	@RequestMapping("store/detail.do")
	public String stroe_detail(HttpServletRequest request,
			HttpServletResponse response)
	{
		// 사용자 => no
		String no=request.getParameter("no");
		...
		// 3,0000원 => 30000
		price=price.replaceAll("[^0-9]", ""); //숫자를 제외한 나머지
		//[^가-힣]   , [가-힣]  , [^A-Za-z]
		vo.setPrice(price);
